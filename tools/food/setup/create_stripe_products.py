...
def create_stripe_products():
    ...
    food_products = [p for p in products_data if p.get('metadata', {}).get('use_case') == 'food_ordering_demo']
    ...
    for product_data in food_products:
        try:
            ...
            if price_info:
                ...
                print(f"Created product: {product.name} (ID: {product.id}) with default price ${price_amount/100:.2f}")
                ...
                print(f"Created product: {product.name} (ID: {product.id}) - No price defined")
                created_products.append({
                    'name': product.name,
                    'id': product.id,
                    'original_id': product_data['id']
